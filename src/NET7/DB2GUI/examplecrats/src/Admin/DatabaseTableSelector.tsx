    ...
    return <>
        <Collapse accordion>
            <Panel header="Simple Search" key="1">
                ...
                {((searchData != null) && searchData.IsValid()) && 
                <>
                    <Link to={`/Admin/Databases/${dtProps.DBName}/tables/${dtProps.TableName}/search/${searchData.ColumnName}/${SearchCriteriaArray[+searchData!.Operator]}/${searchData?.Value}`} target={"_blank"}>Direct Link</Link>
                </>
                    }               
